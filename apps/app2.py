...
def load_covid_data(n_clicks):
	...
	response = requests.get(
		'https://api.covidtracking.com/v1/states/il/daily.json',
		proxies=None,
		verify=False,#avoids SSL errors
	)

	#4) neglected to include the il in the url call to the API which would stipulate your calling values from individual states rather than all states
	...
	table_data = [ #table data is a LIST of DICTIONARY items
		{	'date': row['date'],
			'state': row['state'],
	...
	#5a) appends the list with the new rate (positive / total test) of covid infection, rounds the number to a whole number and formates it as a percentage
	for day in range(len(table_data)):
		pos = table_data[day].get("positive")
		total = table_data[day].get("total")
		rate = round((pos/total), 2)
		rate = "{:.0%}".format(rate)
		table_data[day]['rate'] = rate
